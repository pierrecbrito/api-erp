...
class EmployeeDetail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if email != employee.user.email and User.objects.filter(email=email).exists():
            raise APIException("Esse e-mail já está em uso", code="email_already_use")
        ...
        if groups:
            ...
            for group_id in groups: #1,2,3,4 -> [1, 2, 3, 4]
    ...
    def delete(self, request, employee_id):
        ...
        if check_if_owner:
            raise APIException("Você não pode excluir o dono da empresa")
